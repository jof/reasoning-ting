# reasoning-ting — the full guide 📖

Everything the [README](../README.md) keeps short and sweet, written out in full.
This is the "okay, but how does it *actually* work" document.

## The device

- **EP-2350 "TING"** — RP2350-based handheld mic / sampler / effects unit from
  Teenage Engineering.
- Firmware is **MicroPython 1.25** (custom TE build), app version `EP-2350 1.0.3`
  (the UF2 file is rev `1.0.8`).
- Custom native modules: `ui` (handle / buttons / LEDs / accel / ADC), `spl`
  (sample load / trigger), `fx` (effects chain including a `RING` oscillator).
- `main.py` is **frozen** into the firmware; it reads `/fat/config.json` + WAVs.
- Audio goes out the **3.5 mm analog jack** into the PC's mic input. USB-C is
  control / data only: a CDC-ACM REPL + mass storage ("TINGDISK").
- Connects over USB as `2367:0620`. BOOTSEL ("TING BOOT") shows as
  `Board-ID: RP2350`, the drag-drop UF2 target.

## The plan (chosen approach)
...
Distinct narrowband tones give us an unambiguous press, release, AND submit,
immune to long speech pauses. The PC daemon Goertzel-detects them and synthesizes
a keypress (F12, bound to `/voice`; Enter to submit) into the focused Claude
window.

## Key findings

- `ui.handle()` → 0.0–1.0 float (handle position). The handle is **analog** (ADC
  ch1, rest ~2034/4095); the "two switches" actually read as ADC, not button
  events.
...
- **Power-save clobbers loaded samples.** After 5 min idle (battery only — USB
  ...
  engine back to the ROM stock pack (slot 2 = gunshot, 3 = monkey-boy, 1 =
  alarm). Our Python state survives the wake but **no event fires**, so a
  boot-time `load_tones()` is silently lost and squeeze / release / submit play
  the stock ROM samples until a power-cycle. Fix: `main.py` re-asserts the WAV
  into its slot immediately before *every* trigger (`play()`), which is the only
  wake-proof option since there's nothing to hook. The symptom that pinned it
  down: tones work right after power-on, then revert to gunshot / tarzan once
  it's sat idle.
...
- `listener/` — **the product**: Rust core + the `reasoning-ting` tray GUI and
  `reasoning-ting-listen` CLI (same `reasoning_ting::run` engine). See
...
- `docs/` — [`PACKAGING.md`](PACKAGING.md) (per-platform distribution strategy +
  rationale) and this guide.
...
end-to-end (clean 2525 / 2475 tones, ~290× detection margin).
...
4. **App** — pick one:
   - **Tray GUI** (recommended): `reasoning-ting` (or `nix run .#gui`). Status
     icon (green = standing by, red = voice key held / recording), menu for input
     device / sensitivity / focus-guard / "Write Claude keybinding". Run from a
     terminal to watch the event log
     (`reasoning-ting │ squeeze   voice key down (f12)` …);
...
guard on Linux. Two front-ends — the `reasoning-ting` tray GUI (`--features gui`)
and the `reasoning-ting-listen` CLI — share it. Detector + icon unit-tested
(`cargo test`) and validated against a real capture.
...
**PipeWire** — both build paths route through it (see [`PACKAGING.md`](PACKAGING.md)).
