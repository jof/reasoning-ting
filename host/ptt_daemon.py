...
def tap(key, wid, dry):
    ...
    if wid:
        cmd = ["xdotool", "key", "--window", wid, "--clearmodifiers", key]
    ...
    try:
        while True:
            ...
            if hi > args.threshold and hi > MIN_RATIO * (lo + 1e-9) and now - last_evt > REFRACTORY:
                last_evt = now
                is_intro = m_in >= m_out
                ok, wid = claude_focused(not args.no_focus_guard)
                tag = "INTRO/2525" if is_intro else "OUTRO/2475"
                if not ok:
                    print(f"{tag}  (ignored: no Claude window focused)", flush=True)
                    continue
                if is_intro and not dictating:
                    dictating = True; tap(args.key, wid, args.dry_run)
                    print(f"{tag} -> START (tap {args.key})", flush=True)
                elif (not is_intro) and dictating:
                    dictating = False; tap(args.key, wid, args.dry_run)
                    print(f"{tag} -> SEND  (tap {args.key})", flush=True)
                else:
                    print(f"{tag}  (no-op; dictating={dictating})", flush=True)
